...
const Invest = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            // Fetch baskets from the real API
            const basketsData = await apiService.getBaskets();
            console.log("📊 Loaded baskets:", basketsData);

            // Transform the data to match the expected format
            const transformedBaskets = basketsData.map((basket) => {
                const totalValue = parseFloat(basket.totalValue) || 0;
                const assetCount = basket.assetCount || 0;
                const minRisk = basket.riskRange[0];

                return {
                    id: basket.basketId,
                    basketId: basket.basketId,
                    name: basket.name,
                    description: `Risk Range: ${basket.riskRange[0]}-${basket.riskRange[1]}% • Target APY: ${basket.targetAPY}%`,
                    apy: basket.targetAPY,
                    totalValue: totalValue,
                    availableToInvest: Math.max(0, totalValue * 0.85),
                    assetCount: assetCount,
                    investorCount: Math.floor(Math.random() * 50) + 10, // Mock for now
                    duration: 24, // Mock for now
                    ...
                    composition:
                        basket.assets?.length > 0
                            ? basket.assets.map((asset) => ({
                                  ...
                                  percentage:
                                      Math.floor(100 / assetCount) || 100,
                              }))
                            : [{ type: "No Assets", percentage: 100 }],
                    assets: basket.assets || [],
                    isMock: false,
                    riskRange: basket.riskRange,
                    totalInvested: parseFloat(basket.totalInvested) || 0,
                };
            });
            ...
            const totalAUM = transformedBaskets.reduce(
                (sum, basket) => sum + basket.totalValue,
                0
            );
            const avgAPY =
                transformedBaskets.length > 0
                    ? transformedBaskets.reduce(
                          (sum, basket) => sum + basket.apy,
                          0
                      ) / transformedBaskets.length
                    : 0;
            const liveBaskets = transformedBaskets.filter(
                (basket) => basket.status === "active"
            ).length;
            const totalAssets = transformedBaskets.reduce(
                (sum, basket) => sum + basket.assetCount,
                0
            );
            const totalInvested = transformedBaskets.reduce(
                (sum, basket) => sum + (basket.totalInvested || 0),
                0
            );
            ...
            setPlatformStats({
                totalAUM,
                avgAPY,
                activeInvestors: Math.max(
                    50,
                    Math.floor(totalAssets * 15) + totalInvested / 1000
                ), // Better mock calculation
                liveBaskets,
            });
        } catch (error) {
            ...
            alert(
                "Failed to load investment baskets. Please check your connection and try again."
            );
    ...
    const filteredAndSortedBaskets = baskets
        .filter((basket) => {
            ...
            if (
                filters.safetyLevel &&
                basket.safetyLevel !== filters.safetyLevel
            ) {
                return false;
            }
            if (filters.minAPY && basket.apy < parseFloat(filters.minAPY)) {
                return false;
            }
            if (filters.maxAPY && basket.apy > parseFloat(filters.maxAPY)) {
                return false;
            }
            return true;
        })
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    <p className="text-xl text-gray-600 max-w-3xl mx-auto font-medium">
                        Discover diversified cash flow baskets with transparent
                        safety scoring
                    </p>
                </motion.div>
                ...
                >
                    <div className="bg-blue-600 text-white border-4 border-blue-700 p-6 transform -rotate-1 hover:rotate-0 transition-transform">
                        <div className="text-3xl font-black mb-2">
                            {formatCurrency(platformStats.totalAUM)}
                        </div>
                    ...
                    <div className="bg-emerald-600 text-white border-4 border-emerald-700 p-6 transform rotate-1 hover:rotate-0 transition-transform">
                        <div className="text-3xl font-black mb-2">
                            {platformStats.avgAPY.toFixed(1)}%
                        </div>
                    ...
                    <div className="bg-purple-600 text-white border-4 border-purple-700 p-6 transform -rotate-1 hover:rotate-0 transition-transform">
                        <div className="text-3xl font-black mb-2">
                            {platformStats.activeInvestors.toLocaleString()}
                        </div>
                            ...
                            <div className="flex justify-between items-center mb-12">
                                <div>
                                    ...
                                    <p className="text-gray-600 font-bold">
                                        {filteredAndSortedBaskets.length}{" "}
                                        BASKETS AVAILABLE
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="text-center py-20">
                                    ...
                                    <h3 className="text-2xl font-black text-gray-900 mb-4 uppercase">
                                        {baskets.length === 0
                                            ...
                                            : "NO BASKETS MATCH FILTERS"}
                                    </h3>
                                    <p className="text-gray-600 font-bold mb-8">
                                        {baskets.length === 0
                                            ? "BASKETS WILL APPEAR HERE ONCE ASSETS ARE ADDED TO THE PLATFORM"
...
const BasketCard = ({ basket }) => (
    <Link to={`/baskets/${basket.id}`} className="block group">
        <div className="bg-white border-4 border-gray-900 hover:shadow-[8px_8px_0px_0px_rgba(59,130,246,0.5)] transition-all duration-200 group-hover:-translate-x-1 group-hover:-translate-y-1">
            ...
            <div
                className={`p-6 border-b-4 border-gray-900 ${
                    !basket.isMock
                ...
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            {basket.assetCount > 0 && (
                                <span className="px-3 py-1 text-xs font-black bg-purple-500 text-white border-2 border-purple-600">
                                    {basket.assetCount} ASSETS
                                </span>
                            )}
                        </div>
            ...
            <div className="p-6 bg-white">
                ...
                <button
                    className={`w-full font-black py-4 px-6 uppercase tracking-wide transition-all border-4 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] hover:-translate-x-0.5 hover:-translate-y-0.5 ${
                        basket.assetCount > 0 && basket.totalValue > 0
